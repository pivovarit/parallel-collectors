...
final class Factory {
    ...
    static <T, K, R, C> Collector<T, ?, CompletableFuture<C>> collectingBy(
      ...
      Options.CollectingOption... options) {
        ...
        for (Options.CollectingOption option : options) {
            if (option instanceof Options.Batched) {
                throw new IllegalArgumentException("batching is implicitly enabled for collectors with classifiers");
            }
        }
