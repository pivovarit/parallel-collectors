...
final class Factory {
  ...
  static <T, K, R> Collector<T, ?, CompletableFuture<Stream<Grouped<K, R>>>> collectingBy(
    ...
    return Factory.collectingBy(
        classifier,
        (Function<Stream<Grouped<K, R>>, Stream<Grouped<K, R>>>) i -> i,
        mapper,
        options);
  }
                ...
                .collect(
                    collecting(
                        ...
                        e ->
                            new Grouped<>(
                                e.getKey(),
                                e.getValue().stream().map(mapper.andThen(a -> (R) a)).toList()),
                        options)));
  }
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "it's obligatory to provide parallelism when using batching"));
      return new AsyncParallelCollector.BatchingCollector<>(
          mapper, finalizer, executor, parallelism);
    }
  ...
  static <T, R> Collector<T, ?, Stream<R>> streaming(
      Function<? super T, ? extends R> mapper, Options.StreamingOption... options) {
    requireNonNull(mapper, "mapper can't be null");
          ...
          config
              ...
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "it's obligatory to provide parallelism when using batching"));
      return new AsyncParallelStreamingCollector.BatchingCollector<>(
          mapper, executor, parallelism, ordered);
    }

    return config.parallelism().isPresent()
        ? AsyncParallelStreamingCollector.from(
            mapper, executor, config.parallelism().getAsInt(), ordered)
  ...
  private static Supplier<Executor> defaultExecutor() {
    return () ->
        Executors.newThreadPerTaskExecutor(
            Thread.ofVirtual().name("parallel-collectors-", 0).factory());
  }
}
