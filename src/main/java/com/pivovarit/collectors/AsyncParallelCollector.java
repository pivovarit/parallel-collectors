...
final class AsyncParallelCollector<T, R, C>
  ...
  public static <T, R, C> Collector<T, ?, CompletableFuture<C>> from(
      Function<? super T, ? extends R> task, Function<Stream<R>, C> finalizer, Executor executor) {
  ...
  record BatchingCollector<T, R, C>(
      Function<? super T, ? extends R> task,
      Function<Stream<R>, C> finalizer,
      Executor executor,
      int parallelism)
          ...
          return partitioned(items, parallelism)
              .collect(
                  AsyncParallelCollector.from(
                      ...
                      r -> finalizer.apply(r.flatMap(Collection::stream)),
                      executor,
                      parallelism));
        }
      };
