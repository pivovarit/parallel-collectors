...
class PartitioningIterator<T> implements Iterator<List<T>> {
    ...
    private PartitioningIterator(List<T> list, int numberOfParts) {
        source = list;
        size = list.size();
    ...
    private static <T> Iterator<List<T>> from(List<T> source, int chunks) {
        return new PartitioningIterator<>(source, chunks);
    }
