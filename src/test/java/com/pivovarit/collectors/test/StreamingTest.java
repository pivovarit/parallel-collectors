...
class StreamingTest {
  ...
  private static Stream<
          Factory.GenericCollector<Factory.StreamingCollectorFactory<Integer, Integer>>>
      allStreaming() {
    return Stream.of(
        streamingCollector("parallelToStream()", (f) -> ParallelCollectors.parallelToStream(f)),
        streamingCollector(
            "parallelToStream(e)", (f) -> ParallelCollectors.parallelToStream(f, e())),
        streamingCollector(
            "parallelToStream(e, p)", (f) -> ParallelCollectors.parallelToStream(f, e(), p())),
        streamingCollector(
            "parallelToOrderedStream()", (f) -> ParallelCollectors.parallelToOrderedStream(f)),
        streamingCollector(
            "parallelToOrderedStream(e)",
            (f) -> ParallelCollectors.parallelToOrderedStream(f, e())),
        streamingCollector(
            "parallelToOrderedStream(e, p)",
            (f) -> ParallelCollectors.parallelToOrderedStream(f, e(), p())));
  }

  private static Stream<
          Factory.GenericCollector<Factory.StreamingCollectorFactory<Integer, Integer>>>
      allCompletionOrderStreaming() {
    return Stream.of(
        streamingCollector("parallelToStream()", (f) -> ParallelCollectors.parallelToStream(f)),
        streamingCollector(
            "parallelToStream(e)", (f) -> ParallelCollectors.parallelToStream(f, e())),
                      ...
                      Thread.startVirtualThread(
                          () -> {
                            Stream.concat(Stream.of(0), IntStream.range(1, 10).boxed())
                                .collect(
                                    c.factory().collector(i -> returnWithDelay(i, ofSeconds(i))))
  ...
  void shouldUseSyncFallback() {
    var result =
        Stream.of(1, 2, 3, 4)
            .collect(
                ParallelCollectors.parallelToStream(
                    i -> i,
                    r -> {
  ...
  void shouldUseSyncFallbackForOrdered() {
    var result =
        Stream.of(1, 2, 3, 4)
            .collect(
                ParallelCollectors.parallelToOrderedStream(
                    i -> i,
                    r -> {
                      throw new IllegalStateException("boo!");
                    },
